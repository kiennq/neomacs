...
$vendor = Join-Path $PSScriptRoot 'vendor-windows-gstreamer-runtime.sh'

$buildSource = Get-Content -LiteralPath $build -Raw
$vendorSource = Get-Content -LiteralPath $vendor -Raw
if ($vendorSource -notmatch '\$\{GSTREAMER_ROOT:-\}') {
  throw 'vendor script no longer declares the GSTREAMER_ROOT contract'
}
if ($buildSource -notmatch '\$env:GSTREAMER_ROOT\s*=\s*\$runtimeRoot') {
  throw 'emacs-build does not pass its extracted runtime root as GSTREAMER_ROOT'
}
if ($buildSource -notmatch 'Test-Path\s+-LiteralPath\s+''Env:GSTREAMER_ROOT''') {
  throw 'emacs-build does not preserve a pre-existing GSTREAMER_ROOT'
}
if ($buildSource -notmatch 'Remove-Item\s+Env:GSTREAMER_ROOT\b') {
  throw 'emacs-build does not clear its temporary GSTREAMER_ROOT'
}
...
'@ | Set-Content -LiteralPath (Join-Path $buildHarnessTools 'cargo.cmd')
  ...
  $env:NEOMACS_TEST_CARGO_ARGS = Join-Path $buildHarness 'cargo-args.txt'
  $env:PATH = "$buildHarnessGitCmd;$buildHarnessTools;$env:PATH"
  $packageOutput = @(
    & pwsh -NoProfile -File (Join-Path $buildHarnessScripts 'emacs-build.ps1') `
      -Profile dev-release -SkipBuild 2>&1
  )
  if ($LASTEXITCODE -eq 0 -or ($packageOutput -join "`n") -notmatch 'requires -SkipPackage') {
    throw "dev-release packaging was not rejected: $($packageOutput -join "`n")"
  }
  $buildHarnessOutput = @(
    & pwsh -NoProfile -File (Join-Path $buildHarnessScripts 'emacs-build.ps1') `
      -Profile dev-release -SkipPackage 2>&1
  )
  if ($LASTEXITCODE -ne 0) {
    throw "PowerShell build did not expose Git awk.exe to xtask: $($buildHarnessOutput -join "`n")"
  }
  $cargoArguments = (Get-Content -LiteralPath $env:NEOMACS_TEST_CARGO_ARGS -Raw).Trim()
  if ($cargoArguments -cne 'xtask fresh-build --profile dev-release --features neomacs-layout-engine/freetype-bundled') {
    throw "dev-release did not pass the expected xtask arguments: $cargoArguments"
  }
  $buildHarnessOutput = @(
    & pwsh -NoProfile -File (Join-Path $buildHarnessScripts 'emacs-build.ps1') `
      -SkipPackage 2>&1
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "PowerShell release build did not expose Git awk.exe to xtask: $($buildHarnessOutput -join "`n")"
  }
  $cargoArguments = (Get-Content -LiteralPath $env:NEOMACS_TEST_CARGO_ARGS -Raw).Trim()
  if ($cargoArguments -cne 'xtask fresh-build --release --features neomacs-layout-engine/freetype-bundled') {
    throw "release did not pass the expected xtask arguments: $cargoArguments"
  }
