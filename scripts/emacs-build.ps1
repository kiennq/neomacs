...
param(
  [switch]$SkipBuild,
  [switch]$SkipPackage,
  [ValidateSet('release', 'dev-release')]
...
if ($Profile -ne 'release' -and -not $SkipPackage) {
  throw "profile '$Profile' requires -SkipPackage; packaging supports only release artifacts"
}
...
try {
  ...
  if (-not $SkipBuild) {
    ...
    try {
      ...
      [string[]]$profileArguments = if ($Profile -eq 'release') {
        @('--release')
      } else {
        @('--profile', $Profile)
      }
      & cargo xtask fresh-build @profileArguments --features neomacs-layout-engine/freetype-bundled
  ...
  $requiredArtifacts = @(
    ...
    (Join-Path $releaseDirectory 'cmdproxy.exe')
  )
  ...
  $previousDevelopmentRoot = $env:GSTREAMER_ROOT_X86_64
  $hadGStreamerRoot = Test-Path -LiteralPath 'Env:GSTREAMER_ROOT'
  $previousGStreamerRoot = $env:GSTREAMER_ROOT
  try {
    $env:GSTREAMER_ROOT_X86_64 = $runtimeRoot
    $env:GSTREAMER_ROOT = $runtimeRoot
    ...
    if ($hadGStreamerRoot) {
      $env:GSTREAMER_ROOT = $previousGStreamerRoot
    }
    else {
      Remove-Item Env:GSTREAMER_ROOT -ErrorAction SilentlyContinue
    }
    if ($hadDevelopmentRoot) {
      $env:GSTREAMER_ROOT_X86_64 = $previousDevelopmentRoot
    }
    else {
