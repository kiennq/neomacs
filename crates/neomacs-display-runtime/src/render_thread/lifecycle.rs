...
impl RenderApp {
    pub(super) fn take_primary_realization_request(&mut self) -> bool {
        std::mem::take(&mut self.lifecycle_flags.primary_realization_requested)
    }
    ...
    pub(super) fn handle_about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if self.take_primary_realization_request() {
            self.create_pending_primary(event_loop);
        }
