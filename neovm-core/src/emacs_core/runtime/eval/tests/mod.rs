...
fn failed_aot_prewarm_does_not_insert_or_lower_the_jit_threshold() {
    ...
    assert!(func.runtime.is_aot_prewarmed());
    ...
    // unchanged threshold.
    func.runtime.set_heat_for_test(threshold.saturating_sub(11));
    assert!(matches!(
        func.runtime.dispatch_sized(func.ops.len()),
        Plan::Compiled
    ));
...
fn successful_aot_prewarm_hits_and_executes_on_call_one() {
    ...
    ev.obarray.set_symbol_function_id(sym, function);
    assert!(func.runtime.is_aot_prewarmed());
    assert_eq!(func.runtime.heat(), 0);
    assert!(matches!(
        func.runtime.dispatch_sized(func.ops.len()),
        Plan::Compiled
    ));
    assert!(func.runtime.heat() < hot_threshold());
